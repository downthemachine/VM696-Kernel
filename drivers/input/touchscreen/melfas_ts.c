...
#define TS_LATEST_FW_VERSION_A	0x04
...
static int Set_Download_By_GPIO_FLAG=1;
...
//	struct melfas_ts_data *ts = container_of(work, struct melfas_ts_data, work);
	...
	int ret = 0, i;
	int touchType = 0, keyID = 0, touchID=0, width = 0, strength=10, touchState=0,  posX=0, posY=0, reportID = 0;
	uint8_t buf[TS_READ_REGS_LEN];
	uint8_t read_num;// FingerID;
	bool fTouchedScreen = false;

	if (lgf_key_lock)
	{
		printk("Testmode Key lock on\n");
		return;
	}
	...
	if(ret<=0)
		...
		if( buf[i] == 0x0f ){	//ESD TEST PROTECT CODE
			release_all_finger(ts);
				if(power_flag==1){
					power_flag--;	
			ts->power(0);
		...
		//Ghost Finger Code Start
		//
		//
		//Ghost Finger Code End
...
// LGE_CHANGE_S [myeonggyu.son@lge.com] [2011.02.25] [GELATO] enable or disable key logging status of slate [START]
				...
				if(key_touch_logging_status == 1)
					mtc_send_touch_log_packet(g_Mtouch_info[i].posX, g_Mtouch_info[i].posY, touchState);
#endif
// LGE_CHANGE_E [myeonggyu.son@lge.com] [2011.02.25] [GELATO] enable or disable key logging status of slate [END]
...
void melfas_firmware_info(struct melfas_ts_data *ts,unsigned char *fw_ver, unsigned char *hw_ver, unsigned char *comp_ver)
{
	...
	i2c_smbus_write_byte(ts->client, HARDWARE_REVISION);
	data = i2c_smbus_read_byte(ts->client);
	msleep(10);
	printk(KERN_INFO "HARDWARE REVISION [0x%x]\n", data);
	*hw_ver = data;

	i2c_smbus_write_byte(ts->client, COMPATIBILITY_GROUP);
	data = i2c_smbus_read_byte(ts->client);
	msleep(10);
	...
	*comp_ver = data;
	...
	i2c_smbus_write_byte(ts->client, CORE_FIRMWARE_VERSION);
	data = i2c_smbus_read_byte(ts->client);
	msleep(10);
...
//   	INIT_WORK(&ts->work, melfas_ts_work_func);
	...
	if(fw_ver==0xfa)	//errno.h ENXIO (No such device or address)
	{
		Set_Download_By_GPIO_FLAG=0;
//		#undef SET_DOWNLOAD_BY_GPIO
//		#define SET_DOWNLOAD_BY_GPIO 0
	}else
	{		
		Set_Download_By_GPIO_FLAG=1;
	}
		
//#if SET_DOWNLOAD_BY_GPIO
if(Set_Download_By_GPIO_FLAG==1)
{
...
//	mcsdl_download_binary_data(1, 1,hw_ver,0x01);
	...
	if (fw_ver !=TS_LATEST_FW_VERSION_A) {
